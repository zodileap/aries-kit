...
- 每次提交 Git 时，如果本次改动会进入下一次发布，必须同步补充 `CHANGELOG.md` 中的“更新记录”。
- “更新记录”默认写到**下一个版本号**，不是当前 `package.json` 里的版本号。
  例如：当前版本是 `0.2.10`，则日常开发和提交时应把变更先写到 `0.2.11` 的记录下。
- 当用户明确说“更新版本”时，才真正把版本号升级到下一个版本。
  例如：当前版本是 `0.2.10`，执行“更新版本”后应改为 `0.2.11`。
- 更新记录至少要包含：版本号、日期、主要变更点，以及涉及的组件、构建、文档或发布影响。
- 如果本次对外行为、公开 props、默认值、构建产物或发布流程有变化，必须在版本记录中明确写出；未补齐更新记录前，不允许认为版本更新完成。
- 完成版本升级时，必须同步创建对应的 Git tag，格式为 `<版本号>`，例如 `0.2.11`。
- 版本 tag 必须使用 annotated tag；tag 的消息内容必须直接使用 `CHANGELOG.md` 中对应版本号下的更新内容，至少包含该版本标题及其完整变更条目，不能只写单行摘要。
- 如果本次提交包含版本升级，则提交 Git 时必须保证 **commit 和 tag 一起推送**；不能只提交 commit 不提交 tag。
- 如果用户只说“提交 git”而没有说“更新版本”，则默认只维护下一版本的更新记录，不修改 `package.json` 版本号，也不创建新的版本 tag。
